...
class MessageHandler {
    ...
    /**
     * 根据 mailbox.[mailboxQueryKey]: mailboxQueryValue & mailbox.user 获取 Mailbox
     * @param options
     * @param callback
     * @returns {*}
     */
    getMailbox(options, callback) {
        ...
        // query is Null, then generate query
        if (!query) {
            query = {};
            if (options.mailbox) {// mailbox

                // if mailbox type is object and mailbox._id exist, callback with mailbox directly
                if (typeof options.mailbox === 'object' && options.mailbox._id) {
                    return setImmediate(() => callback(null, options.mailbox));
                }

                // else find by _id and user of mailbox
                query._id = options.mailbox;// mailbox._id
                if (options.user) {
                    query.user = options.user;// mailbox.user
                }
            } else {
                // else find by user and specialUse/path of mailbox
                query.user = options.user;
                if (options.specialUse) {
                    query.specialUse = options.specialUse;// mailboxQueryKey = 'specialUse', mailboxQueryValue = '\\Junk';
                } else {
                    query.path = options.path;// default mailboxQueryKey = 'path', default mailboxQueryValue = 'INBOX'
                }
            }
        }
     ...
     * 这里有两种情况：
     * 一、message存在；
     * 1。skipExisting 为 true 时，
     * 其未对message记录做其它的任何操作，只是更新了outbound（当其存在的时候）
     * <p>
     * callback(null, true, {
     * uid: existingUid,
     * id: existingId,
     ...
     * })
     * <p>
     * 2。skipExisting 为 false 时，
     * - 使用原有的 message 创建一条新的 message 记录（并更新该新的 message 的 _id, uid, modseq, flags），
     * - mailbox 的 uidNext、modifyIndex自增+1，
     * - 删除原有的message记录
     * <p>
     * callback(null, true, {
     * uidValidity: mailboxData.uidValidity,
     * uid: newUid,// new message uid
     * id: existingId,// TODO original message _id
     * mailbox: mailboxData._id,// mailbox _id
     * status: 'update'
     * })
     * <p>
     * 二、mesage不存在；
     ...
     * uidValidity,
     * uid,// new message uid
     * id: messageData._id,// new message _id
     * mailbox: mailboxData._id,
     * status: 'new'
     * });
     *
     * @param options  messageOpts
     * @param callback
     * @returns {*}
     */
    ...
    add(options, callback) {
        // for raw without prepared
        if (!options.prepared && options.raw && options.raw.length > consts.MAX_ALLOWE_MESSAGE_SIZE) {
            return setImmediate(() => callback(new Error('Message size ' + options.raw.length + ' bytes is too large')));
        }

        // if options.prepared is not Null, return back options.prepared immediately
        // otherwise do prepareMessage
        this.prepareMessage(options, (err, prepared) => {
            ...
            this.getMailbox(options, (err, mailboxData) => {
                ...
                this.checkExistingMessage(
                    ...
                    options,
                    (...args) => {
                        if (args[0] || args[1]) {// message exists, update it already
                            return callback(...args);
                        }

                        /**
                         * 删除已保存的 maildata.nodes
                         *
                         * @param args
                         * @returns {*}
                         */
                        ...
                        // message not exists
                        this.indexer.storeNodeBodies(maildata, mimeTree, err => {// 保存 maildata.nodes ，并将保存的 id 存储到 mimeTree.attachmentMap
                            if (err) {
                                return cleanup(err);
                            }
                            ...
                            let messageData = {
                                ...
                                magic: maildata.magic,// TODO what's mean ?
                            ...
                            // TODO relationship with maildata.node & mimeTree.attachmentMap ?
                            ...
                            if (maildata.text) {
                                ...
                                messageData.intro = messageData.text
                                    ...
                                    .replace(/^>.*$/gm, '')
                                    // remove lines with repetetive chars(repeat times at least twice)
                                    // "---------------------"
                                    .replace(/^\s*(.)\1+\s*$/gm, '')
                                    // join lines
                                    .replace(/\s+/g, ' ')
                                ...
                                if (messageData.intro.length > 128) {
                                    ...
                                    let lastSp = intro.lastIndexOf(' ');// last separator
                                    if (lastSp > 0) {
                            ...
                            // increase user's storageUsed by prepared.size
                                                        ...
                                                        () => {
                                                            ...
                                                            this.notifier.addEntries(
                                                                ...
                                                                () => {
                                                                    ...
                                                                    return cleanup(null, true, {// callback(...args)
                                                                        uidValidity,
                                                                        uid,// new message uid
                                                                        id: messageData._id,// new message _id
                                                                        mailbox: mailboxData._id,
    ...
    checkExistingMessage(mailboxData, messageOpts, options, callback) {
        ...
        this.database.collection('messages').findOne(
            {
                mailbox: mailboxData._id,// mailbox._id
                hdate: messageOpts.hdate,// prepared.hdate
                msgid: messageOpts.msgid,// prepared.msgid
                uid: {
                    $gt: 0,
                    $lt: mailboxData.uidNext// mailbox.uidNext
                }
            },
            queryOpts,
            (err, messageData) => {
                ...
                let existingId = messageData._id;// message _id
                let existingUid = messageData.uid;// message uid
                ...
                if (options.skipExisting) {
                    ...
                    // 如果 forward 和 autoreply 产生了 outbound，则需要更新原有 message 的 outbound
                    if (options.outbound) {
                        // new outbound ID's. update
                        return this.database.collection('messages').findOneAndUpdate(
                            {
                                _id: messageData._id,// Unique
                                mailbox: messageData.mailbox,
                                uid: messageData.uid
                            },
                            // $addToSet操作符为数组添加一个值，除非该值已经存在，这种情况下$addToSet对该数组不做任何操作。
                            {
                                $addToSet: {
                            ...
                            {
                                // 如果为false，则返回更新的文档而不是原始文档。 默认值是true。
                                returnOriginal: true,
                                // 限制所有匹配文档返回的字段。
                                projection: {
                                    _id: true,
                ...
                this.database.collection('mailboxes').findOneAndUpdate(
                    ...
                    (err, item) => {
                        ...
                        // as returnOriginal is true, so item.value will be the original mailbox before update
                        let mailboxData = item.value;
                        ...
                        messageData._id = messageOpts.id;// prepared.id
                        messageData.uid = newUid;
                        messageData.modseq = newModseq;
                        messageData.flags = messageOpts.flags;

                        // 如果 skipExisting 为 false，则在原有message的数据的基础上新创建一条message（并更新原有的message的 _id, uid, modseq, flags），
                        // mailbox 的 uidNext、modifyIndex自增+1，
                        // 删除原有的message记录
                        this.database.collection('messages').insertOne(messageData, err => {
                            if (err) {
                                ...
                                err => {
                                    ...
                                    this.notifier.addEntries(
                                        mailboxData,
                                        {
                                            command: 'EXPUNGE',
                                            ignore: options.session && options.session.id,
                                            uid: existingUid,// original message uid
                                            message: existingId,// original message _id
                                            unseen: messageData.unseen,// original message unseen
                                            // modseq is needed to avoid updating mailbox entry
                                            modseq: newModseq// TODO new message modseq
                                        },
                                        () => {
                                            this.notifier.addEntries(
                                                mailboxData,
                                                {
                                                    command: 'EXISTS',
                                                    ignore: options.session && options.session.id,
                                                    uid: messageData.uid,// new message uid
                                                    message: messageData._id,// new message _id
                                                    modseq: messageData.modseq,// new message modseq
                                                    unseen: messageData.unseen// original message unseen
                                                },
                                                () => {
                                                    this.notifier.fire(mailboxData.user);
            ...
            (err, mailboxData) => {
                ...
                pushToArchive((err, archived) => {
                    ...
                    this.database.collection('messages').deleteOne(
                        {
                            _id: messageData._id,
                            // https://github.com/nodemailer/wildduck/issues/51
                            mailbox: messageData.mailbox,
                            uid: messageData.uid
                        },
                        err => {
                                    ...
                                    updateAttachments(() => {
                                        ...
                                        this.notifier.addEntries(
                                            ...
                                            () => {
                                                ...
                                                if (!archived) {
                                                    return this.database.collection('messagelog').insertOne(
                                                        {
                                                            id: messageData.meta.queueId || messageData._id.toString(),
                                                            action: 'DELETED',
                                                            parentId: messageData._id,
     ...
     * @param options
     * @param callback getPreparedMessage#next
     * @returns {*}
     */
    prepareMessage(options, callback) {
        ...
        // see 04MIMETREE.json for details
        // reuse cached mimeTree
        let mimeTree = options.mimeTree || this.indexer.parseMimeTree(options.raw);// raw eval from Buffer.concat(chunks, chunklen);
        ...
        // options.date is always Null
        let idate = (options.date && parseDate(options.date)) || new Date();
        let hdate = (
                        mimeTree.parsedHeader.date &&
                        parseDate([].concat(mimeTree.parsedHeader.date || []).pop() || '', idate)
                    ) || false;
     ...
     * @param subject
     * @param mimeTree
     * @param callback return back threadId
     */
    getThreadId(userId, subject, mimeTree, callback) {
        ...
        this.database.collection('threads').findOneAndUpdate(
            ...
            (err, r) => {
                ...
                // see THREADS.FINDONEANDUPDATE.png
                if (r.value) {
